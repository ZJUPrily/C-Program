...
/*    RegisterWindowClass();*/
    ...
    wndcls.hIcon = LoadIcon(hInst, IDI_ICON_LTAC);
                                         ...
                                         WPARAM wParam, LPARAM lParam)
{
    ...
    switch(msg)
    {
    	...
    	case WM_TIMER:
    		...
    		return 0;
        case WM_DESTROY:
			issave = MessageBox(hwnd, "是否保存当前文件", "提示", MB_YESNO);
			if (issave == 6)
			{
				DoFileSave(hwnd);
				PostQuitMessage(0);
...
/*
    ...
    if(msg == WM_DESTROY)
    {
			issave = MessageBox(hwnd, "是否保存当前文件", "提示", MB_YESNOCANCEL);
			if (issave == 6)
			{
				DoFileSave(hwnd);
				PostQuitMessage(0);
